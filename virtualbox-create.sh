...
echo "Creating LFS hard drive ($LHDSIZE MB)..." >> vm.log
vboxmanage createhd --filename $HOME/'VirtualBox VMs'/$VMNAME/lfs.vdi --size $LHDSIZE >> vm.log
vboxmanage storageattach $VMNAME --storagectl "SATA Controller" \
	--port 0 --device 0 --type hdd \
	--medium $HOME/'VirtualBox VMs'/$VMNAME/lfs.vdi
